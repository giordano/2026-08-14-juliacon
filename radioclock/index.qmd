...
* In central Europe they listen to **DCF77**: **D**eutschland, long wave (**C**), **F**rankfurt, at **77**.5 kHz
* Transmitter in Mainflingen near Frankfurt, operated on behalf of [PTB](https://www.ptb.de/), Germany's national metrology institute
...
* Each minute transmits the date and time of the *following* minute

## The 60-bit format{.wide-slide}
...
* Only two dependencies: `Dates` and `TimeZones`
...
## Radio is noisy: `decode` checks everything
...
* `decode` returns a `ZonedDateTime`, not a naive timestamp
...
* The year is only two digits: the signal is ambiguous, `RadioClock.jl` assumes 2000–2099
...
```{julia}
using TimeZones
talk = ZonedDateTime(2026, 8, 14, 11, 30, tz"Europe/Berlin")
...
Handy for generating test data — or for a transmitter of your own^[check your local radio regulations before powering an antenna at 77.5 kHz...]
...
* Errors are descriptive: they tell you *which* consistency check failed and on what input

## What's next

* More signals behind the same `RadioSignal` abstraction:
    * MSF (60 kHz, UK), WWVB (60 kHz, US), JJY (40/60 kHz, Japan)
* Decoding a live bitstream sampled from a receiver (SDR or sound card)
* Handling the encrypted weather forecast bits is left as an exercise to the reader
...
::: {.column width="60%"}
* DCF77 in, `ZonedDateTime` out — and back
* Every consistency check the signal allows, including time-zone sanity
* Give it a try:
...
[github.com/giordano/RadioClock.jl](https://github.com/giordano/RadioClock.jl)
:::
